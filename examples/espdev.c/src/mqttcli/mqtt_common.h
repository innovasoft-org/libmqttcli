...
/** MAX packet ID  */
#define DEFAULT_MAX_PKT_ID 32

/** Minimum supported MQTT protocol version */
#define MIN_MQTT_PROT_VERSION 4
/** Maximum supported MQTT protocol version */
#define MAX_MQTT_PROT_VERSION 5
/** Minimum supported QOS value */
#define MIN_QOS               0
/** Maximum supported QOS value */
#define MAX_QOS               1
/** Minimum supported value for maximal packet ID */
#define MIN_MAX_PKT_ID        2
/** Maximum supported value for maximal packet ID */
#define MAX_MAX_PKT_ID        128
...
struct mqtt_cli_ctx_cb {
   ...
   */
  void     (*find_property) (const uint8_t tag, const uint8_t *buf, size_t *offset, size_t *used, size_t *length);

  uint16_t (*publish) (const mqtt_cli_ctx_cb_t *self, const mqtt_publish_params_t *params);
  uint16_t (*subscribe) (const mqtt_cli_ctx_cb_t *self, const mqtt_subscribe_params_t *params);
  uint16_t (*unsubscribe) (const mqtt_cli_ctx_cb_t *self, const mqtt_unsubscribe_params_t *params);
  uint16_t (*disconnect) (const mqtt_cli_ctx_cb_t *self, const mqtt_disconnect_params_t *params);
